...
def talkToSpot():
    ...
    # Show GUI status while Spot is "talking"
    status_label.config(text="Spot is listening...")
    ...
    def get_vocal_input():
        ...
        try:
            ...
            status_label.config(text="Couldn't understand. Try again.")
            return None
    ...
    def text_to_speech(text, output_file="output_audio.mp3"):
        try:
            status_label.config(text="Spot is talking...")

            # If file exists and is locked, wait briefly (rare case)
            if os.path.exists(output_file):
                try:
                    os.remove(output_file)  # Try deleting to avoid permission issues
                except PermissionError:
                    time.sleep(1)  # Give OS time to release lock
                    os.remove(output_file)  # Try again
            ...
            status_label.config(text="TTS failed.")
    ...
    try:
        ...
        completion = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You are a happy robotic dog named Spot made by Boston Dynamics. Who is currently used at the University of Missouri-Columbia for research. Jose and Kristofferson Culmer have created the GUI in wich you can speak in."},
                {"role": "user", "content": user_input}
            ]
        )
        spot_response = completion.choices[0].message.content
        text_to_speech(spot_response)
        status_label.config(text="Spot finished talking.")
    except Exception as e:
...
def check_battery_status(root):
    ...
    #Error Detection 
        ...
        battery_label.config(text="Unable To Connect To Robot")
...
def mainInterface():
    ...
    #Talk to Spot Button
    talkSpot = tk.Button(root, text="Talk To Spot", font=('arial', 16), command=talkToSpot, background='#3281a8')
    canvas.create_window(200, 425, width=175, height=50, window=talkSpot)
